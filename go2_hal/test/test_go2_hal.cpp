...
void print_state(go2hal::LowState state)
{
		std::cout << "\33[H\33[2J" <<'\n'; //clear screen
...
int main()
{
	...
	go2hal::LowLevelInterface robot_interface;
	// std::shared_ptr<go2hal::LowLevelInterface> robot_interface;
	// robot_interface = std::make_shared<go2hal::LowLevelInterface>(go2hal::LowLevelInterface());
	// unitree::InitEnvironment(); // memory lock -> Requires sudo

	go2hal::LowState state = {0};
	go2hal::LowCmd lowcmd = {0};
