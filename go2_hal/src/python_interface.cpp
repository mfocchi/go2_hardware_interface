...
PYBIND11_MODULE(pygo2_hal, m) {
  ...
  // Low Level Interface
  py::class_<go2hal::LowLevelInterface>(m, "LowLevelInterface")
    .def(py::init<>())
    .def("receive_observation", &go2hal::LowLevelInterface::ReceiveObservation)
    .def("send_command", &go2hal::LowLevelInterface::SendCommand)
    .def("send_low_command", &go2hal::LowLevelInterface::SendLowCmd);
